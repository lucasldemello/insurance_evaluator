...
RSpec.describe InsuranceManager::HomeInsurance, type: :model do
  let(:user) do
  ...
  describe '#call' do
    it 'creates an insurance score' do
      house_insurance = InsuranceManager::HomeInsurance.new(user)
  ...
  describe '#insurance_type' do
    it 'returns the correct insurance type' do
      house_insurance = InsuranceManager::HomeInsurance.new(user)
      expect(house_insurance.send(:insurance_type)).to eq('home')
    end
  end
  ...
  describe '#ineligible?' do
    context 'when user has no vehicle' do
      it 'returns true' do
        house_insurance = InsuranceManager::HomeInsurance.new(ineligible_user)
