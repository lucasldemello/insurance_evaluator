...
RSpec.describe InsuranceManager::InsuranceStrategy do
  ...
  describe '#is_ineligible' do
    it 'raises NotImplementedError' do
      strategy = described_class.new(user)
      expect { strategy.send(:is_ineligible) }.to raise_error(NotImplementedError)
    end
  end
  ...
  describe '#score_type' do
    context 'when is_ineligible is true' do
      it 'returns INELEGIBLE' do
        strategy = described_class.new(user)
        allow(strategy).to receive(:is_ineligible).and_return(true)
    ...
    context 'when score is <= 0' do
      it 'returns STANDARD' do
        strategy = described_class.new(user)
        allow(strategy).to receive(:is_ineligible).and_return(false)
